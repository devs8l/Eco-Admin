...
const Facilities = () => {
    const [facilityType, setFacilityType] = useState('woodenHut');
    ...
    const handleFileUpload = async (e, index = null) => {
        ...
        try {
            ...
            if (index !== null) {
                ...
                updatedImages = [...facilityData[facilityType]];
                updatedImages[index] = {
                    url: result.url,
            ...
            await updateRoomImages(facilityType, updatedImages);
            setFacilityData(prev => ({
                ...prev,
                [facilityType]: updatedImages
            }));
        } catch (error) {
    ...
    const handleSetFeatured = async (publicId) => {
        try {
            const updatedImages = facilityData[facilityType].map(img => ({
                ...img,
    ...
    return (
        <div className="container mx-auto p-6 max-w-7xl">
            <div className="bg-white rounded-lg shadow-md p-6">
                <h1 className="text-3xl font-bold text-gray-800 mb-6">Facility Image Management</h1>

                {/* Facility Type Selector */}
                <div className="mb-8">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Select Facility Type</label>
                    <div className="flex space-x-4">
                        {['woodenHut', 'camping', 'villaFacility', 'dining', 'swimming', 'photoshoot'].map((type) => (
                            <button
                                key={type}
                                onClick={() => setFacilityType(type)}
                                className={`px-4 py-2 rounded-md font-medium transition-colors ${facilityType === type
                                        ? 'bg-blue-600 text-white'
                                        : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                                    }`}
                            >
                                {type === 'woodenHut' ? 'Wooden Huts' :
                                    type === 'camping' ? 'Camping' :
                                        type === 'villaFacility' ? 'Villa Facility' :
                                            type === 'dining' ? 'Dining' :
                ...
                {isLoading ? (
                    ...
                    <div>
                        <h2 className="text-xl font-semibold text-gray-800 mb-4">
                            {facilityType === 'woodenHut' ? 'Wooden Huts' :
                                facilityType === 'camping' ? 'Camping' :
                                    facilityType === 'villaFacility' ? 'Villa Facility' :
                                        facilityType === 'dining' ? 'Dining' :
                                            facilityType === 'swimming' ? 'Swimming' :
                                                'Photoshoot'} Images Images
                            <span className="ml-2 text-sm font-normal text-gray-500">
                                ({facilityData[facilityType]?.length || 0} images)
                            </span>
                        </h2>

                        {facilityData[facilityType]?.length === 0 ? (
                            <div className="text-center py-12 bg-gray-50 rounded-lg">
                                <p className="text-gray-500">No images uploaded yet</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                {facilityData[facilityType]?.map((image, index) => (
                                    <div
                                        key={image.publicId}
                                        className={`relative group overflow-hidden rounded-lg shadow-md transition-transform hover:shadow-lg ${image.isFeatured ? 'ring-2 ring-blue-500' : ''
                                            }`}
                                    >
                                        <img
                                            src={image.url}
